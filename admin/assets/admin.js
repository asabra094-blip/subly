...
async function loadAdminProducts() {
  ...
  const { data: products, error: productsError } = await supabaseClient
    .from("products")
    .select("id,app_name,account_type,duration,supplier_cost,supplier,active,sort_order")
...
function renderAdminProducts() {
  ...
  container.innerHTML = Object.entries(groups).map(([name, variants]) => `
    <section class="service-group">
      <div class="service-group-head"><div><h3>${escapeHtml(name)}</h3><span>${variants.length} variant${variants.length === 1 ? "" : "s"}</span></div></div>
