...
async function sendTopupTelegramNotification(event,topupId){
 try{
  const {data,error}=await supabaseClient.functions.invoke('send-reseller-notification',{body:{event,topup_id:topupId}});
  if(error){console.error('[SUBLY] reseller Telegram notification failed',error);return false;}
  if(data?.ok===false){console.error('[SUBLY] reseller Telegram notification rejected',data);return false;}
  return true;
 }catch(error){console.error('[SUBLY] reseller Telegram notification error',error);return false;}
}

async function approveTopup(id,button){const request=adminTopups.find(x=>x.id===id);if(!request)return;if(!confirm(`Approve ${money(request.amount)}? Only continue after verifying the payment.`))return;const card=button?.closest('.topup-card'),buttons=card?.querySelectorAll('button')||[];buttons.forEach(x=>x.disabled=true);try{const{data,error}=await supabaseClient.rpc('approve_topup',{p_topup_id:id});if(error)throw error;if(data?.success===false)throw new Error('Could not approve top-up.');await sendTopupTelegramNotification('topup_approved',id);await Promise.all([loadTopups(),typeof loadDashboard==='function'?loadDashboard():Promise.resolve()]);}catch(e){alert(e.message||'Could not approve top-up.');buttons.forEach(x=>x.disabled=false);}}
async function rejectTopup(id,button){const request=adminTopups.find(x=>x.id===id);if(!request)return;if(!confirm(`Reject the ${money(request.amount)} top-up request?`))return;const card=button?.closest('.topup-card'),buttons=card?.querySelectorAll('button')||[];buttons.forEach(x=>x.disabled=true);try{const{data,error}=await supabaseClient.rpc('reject_topup',{p_topup_id:id});if(error)throw error;if(data?.success===false)throw new Error('Could not reject top-up.');await sendTopupTelegramNotification('topup_rejected',id);await loadTopups();}catch(e){alert(e.message||'Could not reject top-up.');buttons.forEach(x=>x.disabled=false);}}
