...
async function loadResellerOrdersPage(){
 ...
 const [cr,pr]=await Promise.all([
  customerIds.length?supabaseClient.from('customers').select('id,first_name,last_name,phone').in('id',customerIds):Promise.resolve({data:[],error:null}),
  productIds.length?supabaseClient.from('products').select('id,app_name,account_type,duration,logo_url').in('id',productIds):Promise.resolve({data:[],error:null})
 ]);
 if(cr.error||pr.error){const relatedError=cr.error||pr.error;console.error('[SUBLY] reseller order related data',relatedError);container.innerHTML=`<div class="empty">${escapeHtml(relatedError.message||'Could not load order details.')}</div>`;return;}
